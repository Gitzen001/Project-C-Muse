...
#define enter 13
#define esc 27
#define red     "\033[0;31m"	//color code shortcut macro ANSI escape codes
...
int signin();	//for login page
void signup();
void forgotPassword();
void initializeAdmin();

void LandingPage();		//after login success
void HomePage();
void MainMenu();
...
int viewSongs();	//functions inside mainmenu
int addSongs();			
int removeSong();		
int viewLikedSongs();		
int addToLikedSongs();		
...
int createPlaylist();		//functions inside playlist
int deletePlaylist();
...
int searchSong(char);		//for search pannel
void demoSongPlay();
void viewAllDemoSongs();

//d. some essential function for validations
...
//e. Global Variables ---------------------------------------------------------------------
int i;		//global declare for loping
...
//g. new demo feature options for playing some downloaded songs from system --------------------
char *songs[] = {"your name.mp3", "story about you.mp3", "demo2.mp3", "demo3.mp3"};
int totalSongs = 4;

//h. Structure for userinfo---------------------------------------------------------------------
...
//i. AddSongs Structure
struct song {				//for song data
    char title[100];
    char artist[50];
};
//j. PlayList Structure
struct playlist {
    char name[50];
...
int main()
{
    ...
    while(1)
    {
        ...
        currentKeyInput = Controls(selected, max, 'v');				//v for vertical movement
        if (currentKeyInput == enter)
        {
            switch (selected)
            {
                case 0:
                    ...
                    Sleep(1000);
                    ThemeArt();
                    loginStatus(signin());		//if success it exits while loop by changing global variable logstatusCheck to 1
                    break;
                case 1:
                    printf(green "\n---  Sign Up selected ---");
                    printf("\nPlease wait...\n" reset);
            ...
            if (logstatusCheck == 1)		//check for login -if success loop to Landing page until logout
            {
                break;
            }
            else
        ...
        {
            selected = currentKeyInput;		//sends value of currentKeyInput (key pressed) to selected
        }
    }
    while (logstatusCheck == 1)		//if login is true then it loops to the landingpage without return else loop to login page
    {
        LandingPage();
        if (logstatusCheck == 0)
        {
...
int signin()
{
    ...
    FILE *p;
	p = fopen("credentials.dat", "rb");			//checks if file exists or not if not no user (if no user then create admin account first)
	if(p == NULL)
    {
    	fclose(p);
...
//sign in validation check
    ...
    printf(green "\n\n--- Sign In Page ---  \n" reset);
    //confirmation
	confirm = keySelectionConfirm("Sign In?");
...
void signup()
{
    ...
    printf(green "\n\n--- Sign Up Page ---  \n" reset);
    //confirmation
    int confirm = keySelectionConfirm("Sign Up?");
	if(confirm == 0) return;
	fflush(stdin);
    ...
    //Signup Confirmation::
    printf("\n\nPress "red"y or Y"reset" to confirm sign_up: \n"reset);
    confirm = getch();
...
void forgotPassword()
{
    ...
    //confirmation
    int confirm = keySelectionConfirm("Forgot Password?");
	if(confirm == 0) return;
	fflush(stdin);
    ...
    printf(red "\nPassword reset for admin is restricted!\n" reset);
    ...
    if(account_found)
    {
        ...
        fseek(p, current_pos, SEEK_SET);		//goes to the location where data is found
...
void LandingPage()
{
	...
	//calls Control function for horizontal mode
        currentKeyInput = Controls(selectedOption, max, 'h');
        if (currentKeyInput == enter)
        {
...
{
	int max = 6;
	if(currentRole == 1)		//if user limit the options to 5
	{
		max = 5;
	}
	else if(currentRole == 2)	//if user limit the options to 6
	{
		max = 6;
	}
    int selectedOption = 0;
    ...
    while(1)
    {
        ...
        printf(yellow"\n\nUse: w-(up), s-(down), enter-(selection), esc-(return))" reset);
        currentKeyInput = Controls(selectedOption, max, 'v');
        if(currentKeyInput == esc)
        {
            return;
        ...
        else if(currentKeyInput == enter)
        {
            switch(selectedOption)
            {
                case 0:			//view all songs
                    viewSongs();	
                    printf(green "\nPress any key to return..." reset);
    				getch();
                    break;
                case 1:			//view liked songs
                	viewLikedSongs();
                	printf(green "\nPress any key to return..." reset);
                	getch();
                    break;
                ...
                case 3:			//remove liked songs
                    ...
                    break;
                case 4:			//add or upload songs by artist and admin only;
                	addSongs();		
                	break;
                case 5:			//delete a song admin only
					viewSongs();
					if(globalSongsFound ==1 )
					{
                    	removeSong();
...
void SearchSongPannel()
{
    ...
    printf("\n1. Search by Name\n2. Search by artist\n");
    	invalidOption:
    	fflush(stdin);
    printf("\nEnter your choice (use esc to return):");
    ch = getch();
    if(ch != '1' && ch != '2' & ch != esc)
    {
...
//9. Search a song by name or artist
...
void Playlists() 
{
    ...
    while (1) 
	{
		...
		else if (currentKey == esc) return;
		else selected = currentKey;
    }
}
...
int createPlaylist() 
{
    ...
    //confirmation
	printf(green "\n\n=== Create Playlist ===\n" reset);
    int confirm = keySelectionConfirm("Create a playlist?");
	if(confirm == 0) return 0;
	fflush(stdin);
	
    // Open the file in read mode to check existing playlists
    ...
    do {
        ...
        fseek(fp, 0, SEEK_SET);
        while (fread(&temp, sizeof(struct playlist), 1, fp) == 1) 
		{
            if (strcmp(temp.name, p.name) == 0) 
			{
                printf(red "Playlist name already exists! Please enter a different name.\n" reset);
                duplicate = 1;
                break;
            }
        }
    ...
    // Now append new playlist
    fp = fopen("playlists.dat", "ab");
    if (fp == NULL) 
	{
        printf(red "Error creating playlist file.\n" reset);
        getch();
...
//12. addSongToPlaylist
int addSongToPlaylist() 
{
    system("cls");
	//confirmation
	printf(green "\n\n=== Add Song to Playlist ===\n" reset);
    int confirm = keySelectionConfirm("Add a song to playlist??");
	if(confirm == 0) return 0;
	fflush(stdin);
    ...
    while (fread(&allPlaylists[count], sizeof(struct playlist), 1, plistFile) == 1 && count < 100) 
	{
        printf("%d. %s\n", count+1, allPlaylists[count].name);
        count++;
    }
    fclose(plistFile);
    ...
    if (count == 0) 
	{
        printf(red "No playlists available.\n" reset);
        getch();
        return 0;
    }

    // Get playlist selection
    printf("\nSelect playlist (1-%d): ", count);
    fflush(stdin);
    int choice;
    scanf("%d", &choice);
...
int viewAllPlaylists() 
{
    system("cls");
    printf(green "\n\n=== View all Playlists ===\n" reset);
    int confirm = keySelectionConfirm("View playlists?");
	if(confirm == 0) return 0;
	fflush(stdin);
    ...
    struct playlist allPlaylists[100], p;
    int count = 0;

    while (fread(&p, sizeof(struct playlist), 1, plistFile) == 1) 
	{
        printf("%d. %s\n", count + 1, p.name);
...
do {
	...
	{
        printf(red "Number out of range! Please enter between 1 and %d.\n" reset, count);
        result = 0; // set result = 0 to repeat the loop
    }
    ...
    FILE *pf = fopen(playlistFilename, "rb");
    if (pf == NULL) 
	{
        printf(red "Could not open selected playlist file.\n" reset);
        getch();
        return 0;
    }
    ...
    struct song s;
    int songCount = 0;
    while (fread(&s, sizeof(struct song), 1, pf) == 1) 
	{
        printf("%d. %s - %s\n", ++songCount, s.title, s.artist);
    }

    if (songCount == 0)
        {
        	printf(yellow "This playlist is empty.\n" reset);
		}
...
//14. removeSongsfromPlaylist
int removeFromPlaylist() 
{
    system("cls");
    printf(green "\n\n=== Remove Songs from Playlist ===\n" reset);
    int confirm = keySelectionConfirm("Remove songs form playlist?");
	if(confirm == 0) return 0;
	fflush(stdin);
...
int deletePlaylist() 
{
	system("cls");
	printf(green "\n\n=== Delete Playlist ===\n" reset);
    int confirm = keySelectionConfirm("Delete a Playlist?");
	if(confirm == 0) return 0;
	fflush(stdin);
    ...
    int choice;
    printf("\nEnter the playlist number to delete: ");
    ...
    printf(red "Are you sure you want to delete playlist '%s'? (y = Yes, n = No): " reset, allPlaylists[choice - 1].name);
  	fflush(stdin);
    scanf(" %c", &confirm);
...
//16. addSongs function()
int addSongs() 
{
    struct song newSong;
    //confirmation
    int confirm = keySelectionConfirm("Upload a Song?");		//parameter is for message display
    if(confirm == 0) return 0;
...
//17. viewSongs Function(), for displaing all added songs
int viewSongs() 
{
    FILE *p;
    struct song s;
...
int removeSong() 
{
    ...
    //confirmation first
	printf(green "\n\n=== Delete Music ===\n" reset);
    int confirm = keySelectionConfirm("Delete a Song?");
	if(confirm == 0) return 0;
	fflush(stdin);			//new trailing detected so used for removing that with a extra enter to be safe
...
//19. function for  adding songs to liked
int addToLikedSongs() 
{
    char title[100];
    struct song s;
    ...
    //confirmation
    int confirm = keySelectionConfirm("Like a Song?");
	if(confirm == 0) return 0;
	fflush(stdin);
...
//20. functions for displaying all liked songs if any
int viewLikedSongs() 
{
    int count = 0;
    struct song s;
...
int removeLikedSong() 
{
	...
	//confirmation
	printf(green "=== \n\nRemove Liked Music ===\n" reset);
    int confirm = keySelectionConfirm("Remove Liked Song?");
	if(confirm == 0) return 0;
	fflush(stdin);
...
//22. Controls For Console Selections
int Controls(int currentSelection, int maxOptions, char direction)
{
    int invalidKeyPressFlag = 0;
    int key_pressed;
invalid:
    key_pressed = getch();
	if (key_pressed == 0 || key_pressed == 0xE0) key_pressed = getch();	// 0xE0 Handle extended keys like arrow key 
    ...
    if (direction == 'v')
    {
        ...
        else if (key_pressed == 13) return 13;
        else if (key_pressed == 27) return 27;
        else invalidKeyPressFlag = 1;

        if (invalidKeyPressFlag == 1)
        {
            printf(red "\nInvalid keypress! Use: w-(up), s-(down), enter-(selection), esc-(return)\n" reset);
            goto invalid;
        }
    }
    else if (direction == 'h')
    {
        ...
        if (invalidKeyPressFlag == 1)
        {
            printf(red "\nInvalid keypress! Use: a-(left), d-(right), enter-(selection), esc-(return)\n" reset);
            goto invalid;
        }
    }
...
void demoSongPlay()
{
    ...
    while (1)
    {
        ...
        printf(green "\nNow Playing: %s\n" reset, songs[index]);
        char command[200];
        sprintf(command, "start /min vlc.exe \"%s\"", songs[index]);
        system(command);
        int playTime = 0;
        while (!kbhit())
        {
        ...
        mediaButton = Controls(index, maxSongs, 'h');
        if (mediaButton == esc) break;
        else index = mediaButton;
...
//25. to confirm the selection
int keySelectionConfirm(char displayMessage[])
{
	printf(green "\n%s\n"reset"Press -enter- to confirm or -any key- to return: ",displayMessage);
    char choice = getch();
    if(choice != 13)
    {
    	puts("return");
